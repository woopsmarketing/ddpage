...
import TestclientClient from "./TestclientClient";

const TESTCLIENT_SLUG = "testclient";

// page-owns-data 패턴 (D-18) — preview.html 의 FAQ 8개를 그대로 박는다.
...
export async function generateMetadata(): Promise<Metadata> {
  return buildPageMetadata({
    slug: TESTCLIENT_SLUG,
    pathname: "/",
    fallback: {
      title: "뚝딱페이지 — 구글에 노출되고 챗지피티가 추천하는 사이트, 14,900원",
      description:
        "검색에서도, AI 답변에서도 노출되는 랜딩페이지. 1인 사업자와 소상공인을 위한 14,900원 올인원 패키지.",
    },
  });
}
...
export default async function TestclientPage() {
  ...
  const host = h.get("host") ?? "testclient.ddpage.kr";
  const config = await loadClient(TESTCLIENT_SLUG);
