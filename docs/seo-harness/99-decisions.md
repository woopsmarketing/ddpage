...
| D-26 | 2026-05-21 | 이미지 자동 압축 + next/image 강제 | **2-layer 이미지 최적화**: (1) `client-integrate` Stage 2 에서 `sharp` 로 자동 압축 — 폭 1000px resize + PNG palette + quality 82 JPEG, 200KB 초과 이미지만 대상, 평균 70% 절감. (2) `page-converter` Mode D 가 처음부터 `<img>` 대신 `<Image>` (`next/image`) 로 변환 — sizes 속성 + lazy load + width/height. hero 이미지는 `priority`. | next/image 만 적용 (Vercel 자동 변환) / sharp 압축 만 적용 (next/image 없이) / 그대로 두고 클라이언트에게 압축 요청 | testclient 의 portfolio 8장 = 9.3MB (2822×16384px PNG 페이지 스크린샷) → LCP 12초 모바일. 사전 압축으로 9.3MB → 2.9MB (-69%), next/image 의 모바일 srcset 으로 추가 ~80% 절감. 클라이언트가 어떤 사이즈로 줘도 인프라가 자동 처리 = 운영 부담 0. |
...
- 2026-05-21: testclient 모바일 PageSpeed 72점 잔존 LCP 12s 진단 후 v1.4 — D-26 (이미지 자동 압축 + next/image 강제). testclient + 메인 portfolio 8장 압축 9.3MB → 2.9MB. Stage 2 자동화 + Mode D 가 처음부터 next/image 사용.
